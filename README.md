...
- IndexedDB persistence (`review-v1`)
- Optional Yjs room `review` on `ws://<host>:1234`
...
Sync is optional. `npm run server` is enough if you only want the websocket. Two browsers on the same machine, or another device on the LAN using your host IP instead of `localhost`, share room `review`.
