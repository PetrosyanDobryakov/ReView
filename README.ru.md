...
Репозиторий и продукт называются ReView. Имя на экране можно переименовать кликом по нему в шапке.
...
- Сохранение в IndexedDB (`review-v1-<id>`)
- Опциональная комната Yjs `review-<id>` на `ws://<host>:1234`
...
Синхронизация необязательна. `npm run server` — только websocket. Два браузера на одной машине или другое устройство в LAN по IP хоста вместо `localhost` сидят в комнате `review-<id>`.
