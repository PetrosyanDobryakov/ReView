...
- Сохранение в IndexedDB (`review-v1`)
- Опциональная комната Yjs `review` на `ws://<host>:1234`
...
Синхронизация необязательна. `npm run server` — только websocket. Два браузера на одной машине или другое устройство в LAN по IP хоста вместо `localhost` сидят в комнате `review`.
