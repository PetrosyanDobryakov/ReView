...
Синхронизация необязательна. `npm run server` — только websocket. Два браузера на одной машине или другое устройство в LAN по IP хоста вместо `localhost` сидят в комнате `doska`. Сервер синхронизации без авторизации. Любой в сети, кто достучится до `:1234`, попадёт в эту комнату.
