...
Доска is a local-first board: draw, shape, text, stickers, images, erase; optional realtime collab via Yjs. Success is a board that stays usable offline, syncs when the local server is up, and does not feel like a generic AI dashboard.
...
- Keep product name **Доска**. Do not rebrand to ReView in the UI.
...
- IndexedDB key `doska-v1` and WS room `doska` stay unless a migration is explicit.

## Brand Commitments

Name: Доска. Incumbent UI language: Russian. GitHub repo name ReView is not the product name. Contributor must not replace identity copy with a fork brand.
