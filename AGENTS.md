...
- **Net debug logs:** **off by default.** Enable with `?netLog=1`, Settings → System → Connection, or `localStorage.setItem('review-net-log','1')`. Console `[review:net]` + session files under `logs/net/` (written by the sync server). Pointers: `logs/net/CURRENT`, `logs/net/latest.log`, and `logs/net/session-<stamp>.log`. Client POSTs batches to `http://<sync>:1234/net-log`; server also records ws connect/disconnect. `GET /net-log` returns the active file path. Empty rooms are GC'd after ~5 minutes (`REVIEW_ROOM_GC_MS`).
