...
- **Workers Builds deploy:** the SPA Worker’s Deploy command must be `npx wrangler deploy`. If it is `echo done`, Builds will green-check while https://review.zpro-driftman.workers.dev/ keeps serving an old hashed bundle (seen stuck on **0.14.25** through tip 0.14.34). Verify with `node scripts/check-live-version.mjs` or Home `v{version}+{sha}` / `<meta name="review-build">`.
