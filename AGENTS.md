...
- **Workers Builds deploy:** Worker **review** Deploy command must be `bash scripts/deploy.sh all` (or at least `npx wrangler deploy`). If it is `echo done`, Builds green-check while https://review.zpro-driftman.workers.dev/ keeps an old bundle. Dashboard-only fix (Builds API is read-only for agents). Verify with `node scripts/check-live-version.mjs` or Home `v{version}+{sha}` / `<meta name="review-build">`.
