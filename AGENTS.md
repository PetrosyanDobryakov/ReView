...
- **Tip deploy:** `bash scripts/deploy.sh all` (or `npm run cf:deploy:all`) publishes **review-sync** then **review**. `review-sync` is the Rust wasm Durable Object (`worker/`, `worker-build`); `worker/src/index.ts` is not the uploaded module. Prefer `CLOUDFLARE_API_TOKEN` + `CLOUDFLARE_ACCOUNT_ID=3058d81da41b02e06744d5d058570aab` in CI/agents; wrangler OAuth is the local fallback. See `docs/DEPLOY.md`.
...
- **Workers Builds rustup:** the Builds image does not include Rust. `scripts/build-sync-worker.sh` (invoked by `deploy.sh all` via wrangler `[build]`) installs rustup stable + `wasm32-unknown-unknown` into `$HOME/.cargo` when `rustup` is missing, then `worker-build` 0.8.6. OpenSSL headers are preinstalled (`libssl-dev`); the script finds them even when `pkg-config` does not. Do not rely on a dashboard install command for Rust. As of 2026-09-25, production Deploy was already `bash scripts/deploy.sh all` and failed on missing rustup (`main` builds `510b0e25…`, `13f8f6a6…` at `60b9eb5`); non-prod Deploy was still `echo done`.
