...
export class SelectTool extends Tool {
  ...
  onDown(engine: Engine, p: PointerInfo): void {
    ...
    if (rotHit) {
      // easter egg — triple click on rotation handle (spawn at live knob world pos)
      const origin = engine.rotateHandleWorldPos(rotHit) ?? p.world;
      engine.handleRotateClick(origin.x, origin.y);
      if (rotHit === '__group__') {
